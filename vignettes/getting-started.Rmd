...
# Overview
    ...
    * A PCS code is best understood as the _result of a process_, not an assigned number.
...
The machinery of PCS is contained in three parts: the **Index**, the **Tables**, and the **List of Codes**. Additionally, there are several **appendices** and a list of guidelines.
...
Choosing a specific value for each of the seven axes derives the code. Based on details about the procedure performed, values are assigned for each character specifying the 
...
The meaning of each character is a function of its placement in the code sequence. As such, the same value in a different position completely alters the meaning of the code:
...
ICD-10-PCS is composed of 17 sections in total. The numeric sections (0 - 9) comprise the **Medical and Surgical-Related** sections. The alphabetic sections (B, C, D, F, G, H, X) comprise the **Ancillary** sections. 
...
A PCS code is built by selecting the appropriate characters to describe the _intent_, the _approach_, and other _qualifying information_ on how the procedure is performed. The example below describes the excision of a dorsal ganglion located at the wrist level.
