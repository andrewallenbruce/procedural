...
x$axes |>
  ...
  tab_header(subtitle = html("<b><i>ICD-10-PCS 2024</i></b>"),
             title = html(paste0("<b><u>", x$code, "</u></b> ", x$description))) |>
  tab_footnote(
    footnote = html(paste0("<b>Definition:</b> ", 
    procedural:::delister(x$definitions[1, 6]))),
    locations = cells_body(columns = label, rows = 3)) |>
  tab_footnote(
    footnote = html(paste0("<b>Definition:</b> ", 
    procedural:::delister(x$definitions[2, 6]))),
...
x$axes |>
  ...
  opt_all_caps() |>
  opt_table_outline(style = "none") |>
  tab_header(subtitle = htmltools::tagList(htmltools::tags$div(
    style = htmltools::css(`text-align` = "right"), 
    htmltools::tags$div(htmltools::tags$strong("ICD-10-PCS 2024")))),
    title = html(paste0("<b>", x$code, "</b>  ", x$description))) |>
  tab_options(
    stub.background.color = "#0B0B35",
    stub.border.color = "#0B0B35",
...
Additional differences include these uses of the 6th character:
   ...
   + Sections 4 - 5 define it as **Function**.
   + Sections 7 - 9 define it as **Method**.
...
The definitions of some characters in the ancillary sections differ from that seen in previous sections. 

In the Imaging section, the 3rd character is defined as root type, and the 5th and 6th characters define contrast and contrast/qualifier respectively, as in the CT scan example below.
...
x$axes |>
  ...
  tab_footnote(
    ...
    locations = cells_body(columns = label, rows = 3)) |>
  tab_footnote(
    footnote = html(paste("<b>Includes:</b>", procedural:::delister(x$includes[1, 5]))),
